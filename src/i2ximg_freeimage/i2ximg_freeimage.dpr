...
uses
  ...
  Classes,
  Windows,
  ui2ximg_FreeImage in 'ui2ximg_FreeImage.pas',
  ...
  FreeImage in 'FreeImage.pas',
  FreeImage32 in 'FreeImage32.pas',
  FreeBitmap in 'FreeBitmap.pas',
...
const
  NAME='FreeImage Image Plugin for I2X';
  SHORT_NAME='FI';
  VERSION = '00.00.02';
type
  TOnEvent = procedure();
...
{$R *.res}
var
  oI2XImgFreeImage : TI2XImgFreeImage;
  ParmFileName : string;
...
begin
  InitCheck();
  Result := -1;
  StrLCopy( lpErrorMessage, PCHar( oI2XImgFreeImage.LastErrorString ), lMaxLen );
  Result := StrLen( PCHar( oI2XImgFreeImage.LastErrorString ) );
end;

function GetLastErrorCode() : integer; stdcall;
begin
  InitCheck();
  Result := oI2XImgFreeImage.LastErrorCode;
end;
...
begin
  ...
  try
    ...
    if ( oI2XImgFreeImage.ProcessImage( sMemoryMapName, Inst )) then
      Result := 1
    else
      Result := 0;
  finally
...
begin
  ...
  Result := oI2XImgFreeImage.GetCapabilities( capabilities );
  StrLCopy( pInstructionList, PChar( capabilities.asString ), lMaxLen );
end;

procedure _Initialize();
begin
  oI2XImgFreeImage := TI2XImgFreeImage.Create( GUID, ParmFileName );
  Descr :='Image2XML FreeImage Plugin Module';
end;

procedure _Terminate();
begin
  FreeAndNil( oI2XImgFreeImage );
end;
