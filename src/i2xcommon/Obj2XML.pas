...
uses Windows, Classes, JclStrings;

procedure ObjectToXML(const FileName: string; Instance: TObject);
procedure XMLToObject(const FileName: string; Instance: TObject);
function ObjectToXMLStr( Instance: TObject ) : string;
procedure XMLStrToObject( const XMLString : string; Instance: TObject );
...
begin
  ...
  try
    intSize := Length( XMLString );
    SetTextStr(XMLString, intSize);
  finally
  end;
end;
