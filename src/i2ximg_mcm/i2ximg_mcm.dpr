...
uses
  ...
  ui2ximg_mcm in 'ui2ximg_mcm.pas',
...
const
  NAME='MCMImage Image Plugin for I2X';
  SHORT_NAME='MCM';
  VERSION = '00.00.02';
type
  TOnEvent = procedure();
...
begin
  InitCheck();
  Result := -1;
  StrLCopy( lpErrorMessage, PCHar( oI2XImgMCM.LastErrorString ), lMaxLen );
  Result := StrLen( PCHar( oI2XImgMCM.LastErrorString ));
end;

function GetLastErrorCode() : integer; stdcall;
begin
  InitCheck();
  Result := oI2XImgMCM.LastErrorCode;
end;
...
begin
  ...
  try
    ...
    oI2XImgMCM.ProcessImage( sMemoryMapName, Inst );
    Result := oI2XImgMCM.LastErrorCode;
    //if ( oI2XImgMCM.ProcessImage( sMemoryMapName, Inst )) then
    //  Result := ERROR_OK
    //else
    //  Result := oI2XImgMCM.LastErrorCode;
  finally
...
begin
  ...
  Result := oI2XImgMCM.GetCapabilities( capabilities );
  StrLCopy( pInstructionList, PChar( capabilities.asString ), lMaxLen );
end;

procedure _Initialize();
begin
  oI2XImgMCM := TI2XImgMCM.Create( GUID, ParmFileName );
  Descr := NAME;
end;

procedure _Terminate();
begin
  FreeAndNil( oI2XImgMCM );
end;
